public class MadLibs {
    /*
  This program generates a mad libbed story.
  Author: Vadzim
  Date: 4/17/2023
    */
    public static void main(String[] args){
        String name1 = "Julia";
        String name2 = "Vadzim";
        String adjective1 = "magnificent";
        String adjective2 = "horrendous";
        String adjective3 = "sunny";
        String verb1 = "run";
        String noun1 = "problem";
        String noun2 = "device";
        String noun3 = "notes";
        String noun4 = "certification";
        String noun5 = "dog";
        String noun6 = "game";
        int number = 15;
        String place1 = "Bloor street";

        //The template for the story
        String story =
                "This morning "+name1+" woke up feeling "+adjective1+". " +
                "'It is going to be a "+adjective2+" day!' Outside, a bunch of " +
                noun1+"s were protesting to keep "+noun2+" in stores. They began to "+verb1 +
                " to the rhythm of the "+noun3+", which made all the "+noun4+"s very "+
                adjective3+". Concerned, "+name1+" texted "+name2+", who flew "+name1 +
                " to "+place1+" and dropped "+name1+" in a puddle of frozen "+noun5 +
                ". "+name1+" woke up in the year "+number+", in a world where "+noun6 +
                "s ruled the world.";

        System.out.println(story);
    }
}
